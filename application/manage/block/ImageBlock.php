...
class ImageBlock extends BaseBlock
{
    ...
    public static function fetch($data = [])
    {
        ...
        $html .= '<div class="am-form-group am-form-file am-text-center nd-input-image-area" style="background-image: url(\'' . $data['value'] . '\')" id="preview_div_' . $data['uuid'] . '">';
        $html .= '<button type="button" class="am-btn am-btn-default am-btn-sm">';
        $html .= '<i class="am-icon-cloud-upload"></i>';
        $html .= '<span>选择文件</span>';
        $html .= '</button>';
        $html .= '<input type="file" class="nd-upload-file" target="upload_file_' . $data['uuid'] . '" preview="preview_div_' . $data['uuid'] . '">';
        $html .= '</div>';
